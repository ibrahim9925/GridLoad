-- Fix ABC Analysis Function - resolve ambiguous column reference (corrected syntax)
...
BEGIN
  ...
  SELECT SUM(consumption_data.annual_value) INTO total_value
  FROM (
    SELECT 
      p.id,
  ...
  with_percentages AS (
    SELECT 
      ...
      (consumption_data.annual_consumption_value / NULLIF(total_value, 0) * 100) as percentage_of_total,
      SUM(consumption_data.annual_consumption_value / NULLIF(total_value, 0) * 100) OVER (ORDER BY consumption_data.annual_consumption_value DESC) as cumulative_percentage
