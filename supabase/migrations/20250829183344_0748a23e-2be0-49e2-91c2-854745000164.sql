...
BEGIN
    ...
    IF NOT col_exists THEN
        -- Use cost_price for both cost and selling if selling_price doesn't exist
    ...
    ELSE
        -- Use proper selling_price column
        INSERT INTO public.products (
            name, 
            category, 
            cost_price, 
        ...
        ('TEST_Solar_Panel_High_Stock', 'Solar Panels', 200.00, 400.00, 150, 20, 50, true, 24, true),
        ('TEST_Inverter_High_Stock', 'Inverters', 500.00, 800.00, 100, 10, 25, true, 36, true),
        ('TEST_Battery_High_Stock', 'Batteries', 800.00, 1200.00, 80, 15, 30, false, 12, true),
        ('TEST_Cable_High_Stock', 'Cables', 50.00, 80.00, 200, 50, 100, false, 0, true),
        ('TEST_Mounting_High_Stock', 'Mounting Systems', 150.00, 250.00, 120, 20, 40, true, 12, true)
        ON CONFLICT (name) DO UPDATE SET
            current_stock = EXCLUDED.current_stock,
            cost_price = EXCLUDED.cost_price,
