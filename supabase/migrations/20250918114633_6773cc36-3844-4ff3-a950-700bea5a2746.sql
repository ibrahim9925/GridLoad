-- Fix stock levels first, then implement Phase 1: Core Data Infrastructure

-- Update all products to have adequate stock levels
UPDATE public.products SET 
  current_stock = CASE 
    WHEN current_stock < 50 THEN 100 + (RANDOM() * 200)::INTEGER
    ELSE current_stock
  END,
  on_hand_qty = CASE 
    WHEN on_hand_qty < 50 THEN 100 + (RANDOM() * 200)::INTEGER
    ELSE on_hand_qty
  END;
...
UPDATE public.containers SET
  status = CASE 
    ...
    WHEN RANDOM() > 0.7 THEN 'in_transit'::container_status_enum
    WHEN RANDOM() > 0.4 THEN 'customs_processing'::container_status_enum
    ELSE status
  END,
  total_cost = CASE 
    ...
    ELSE COALESCE(total_cost, 200000) + (RANDOM() * 300000)::INTEGER
  END,
  expected_arrival_date = CASE 
    ...
    ELSE CURRENT_DATE + (RANDOM() * 30 + 5)::INTEGER
  END,
