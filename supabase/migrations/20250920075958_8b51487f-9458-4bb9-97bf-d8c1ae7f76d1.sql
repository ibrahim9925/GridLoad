-- Add sample data with proper enum casting (Part 2 - Fixed)
...
-- 2. Add comprehensive sample data for testing with proper enum casting
DO $$
DECLARE
  customer_id uuid;
  product_id uuid;
      ...
      IF (i % 3 = 0) THEN
        INSERT INTO sales (
          ...
          CASE 
            ...
            WHEN RANDOM() > 0.4 THEN 'partial_paid'::payment_status
            ELSE 'pending'::payment_status
          END,
          CASE 
            WHEN RANDOM() > 0.6 THEN 'delivered'::fulfillment_status
            WHEN RANDOM() > 0.3 THEN 'processing'::fulfillment_status
...
BEGIN
  ...
  IF supplier_id IS NOT NULL THEN
    ...
    (supplier_id, 'CONT-001', '40ft', 'in_transit'::container_status_enum, CURRENT_DATE - 30, CURRENT_DATE + 5, 85000),
    (supplier_id, 'CONT-002', '20ft', 'ordered'::container_status_enum, CURRENT_DATE - 10, CURRENT_DATE + 25, 45000),
    (supplier_id, 'CONT-003', '40ft', 'port_arrival'::container_status_enum, CURRENT_DATE - 5, CURRENT_DATE - 2, 92000),
    (supplier_id, 'CONT-004', '20ft', 'customs_processing'::container_status_enum, CURRENT_DATE - 3, CURRENT_DATE - 1, 38000)
